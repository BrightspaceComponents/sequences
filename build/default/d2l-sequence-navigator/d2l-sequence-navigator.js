...
import "../node_modules/@brightspace-ui-labs/accordion/accordion.js";
import "../node_modules/@brightspace-ui/core/components/colors/colors.js";
import "../node_modules/siren-entity/siren-entity.js";
...
class D2LSequenceNavigator extends mixinBehaviors([D2L.PolymerBehaviors.Siren.EntityBehavior, D2L.PolymerBehaviors.Sequences.LocalizeBehavior], PolymerElement) {
  static get template() {
    return html`
		<style>
		:host {
...
<!--		<siren-entity href="[[rootHref]]" token="[[token]]" entity="{{_lessonEntity}}"></siren-entity>-->
  ...
  ready() {
    super.ready();
    const styles = this.dataAsvCssVars && JSON.parse(this.dataAsvCssVars) || JSON.parse(document.getElementsByTagName('html')[0].getAttribute('data-asv-css-vars'));
  ...
  _getTopicEntities(entity) {
    return entity && entity.getSubEntities().filter(subEntity => (subEntity.properties && Object.keys(subEntity.properties).length > 0 || subEntity.href) && !subEntity.hasClass('unavailable')).map(this._getHref);
  }
