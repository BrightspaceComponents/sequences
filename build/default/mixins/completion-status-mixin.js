...
export function CompletionStatusMixin() {
  return class extends mixinBehaviors([D2L.PolymerBehaviors.Siren.EntityBehavior, D2L.PolymerBehaviors.Sequences.LocalizeBehavior], PolymerElement) {
    ...
    _getCompletionRequirement(activity) {
      ...
      if (subEntity && subEntity.getSubEntityByClass('exemption') && subEntity.getSubEntityByClass('exemption').hasClass('exempted')) {
        return 'exempt';
      }
    ...
    _getCompletionStatus(activity) {
      ...
      return completionClass.hasClass('completed') ? 'complete' : 'incomplete';
    }
